...
def error_page(request_dict, function_url_list):
    ...
    for k in request_dict.keys():
        if k != "Cookie":
            header_list += f"<li> {k}={request_dict[k]}</li>\n"
    cookie_list = ""
    ...
    with open("templates/error/index.html") as index:
        return http_ok_header([("bardia1", 4), ("bardia2","rtrt")])+index.read().replace("{{LIST}}", list).replace("{{URL}}", request_dict['url']).replace("{{header_list}}", header_list).replace("{{cookie_list}}", cookie_list)